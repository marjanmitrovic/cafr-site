...
async function saveEvidenceAttachments(files, folder) {
  ...
  await fs.mkdir(path.join(EVIDENCE_UPLOAD_DIR, folder), { recursive: true });
  ...
  for (const file of files) {
    ...
    const baseName = sanitizeFileName(path.basename(originalName, path.extname(originalName)) || 'attachment');
    const storedName = `${Date.now()}-${crypto.randomBytes(6).toString('hex')}-${baseName}${extension}`;
    const relativeUrl = `/uploads/evidence/${folder}/${storedName}`;
    const storedPath = path.join(EVIDENCE_UPLOAD_DIR, folder, storedName);
...
app.post('/api/admin/documents/upload', requireAdmin, async (req, res) => {
  try {
    ...
    res.status(201).json({ document: documentResponse(document), file: { url, size: decoded.buffer.length, mime: decoded.mime } });
  } catch (error) {
